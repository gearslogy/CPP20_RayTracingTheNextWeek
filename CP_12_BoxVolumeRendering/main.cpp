...
inline auto cornell_box_transformation(){
    HittableList objects;
    auto red   = std::make_shared<Lambertian>(Color(.65, .05, .05));
    auto white = std::make_shared<Lambertian>(Color(.73, .73, .73));
    auto green = std::make_shared<Lambertian>(Color(.12, .45, .15));
    auto light = std::make_shared<Constant>(Color(15, 15, 15));
    objects.add(std::make_shared<YZ_Rect>(0,555,0,555,555, green)); // right
    objects.add(std::make_shared<YZ_Rect>(0,555,0,555,0, red)); // left
    objects.add(std::make_shared<XZ_Rect>(0, 555, 0, 555, 0, white)); // bottom
    objects.add(std::make_shared<XZ_Rect>(0, 555, 0, 555, 555, white)); //top
    objects.add(std::make_shared<XY_Rect>(0, 555, 0, 555, 555, white)); // front
    ...
    box1 = std::make_shared<Hittable_Translate>(box1, Vec3{265,0,295});
    objects.add(box1);

    std::shared_ptr<Hittable>  box2 = std::make_shared<Box>(Vec3{0,0,0}, Vec3{165,165,165}, white);
    box2 = std::make_shared<Hittable_Rotate_Y>(box2, -18);
    box2 = std::make_shared<Hittable_Translate>(box2,Vec3{130,0,65});
    objects.add(box2);
    return objects;
}
inline auto cornell_box_volume_transformation(){
    HittableList objects;
    auto red   = std::make_shared<Lambertian>(Color(.65, .05, .05));
    auto white = std::make_shared<Lambertian>(Color(.73, .73, .73));
    auto green = std::make_shared<Lambertian>(Color(.12, .45, .15));
    ...
    auto boxVolumeShader = std::make_shared<Isotropic>(Vec3{1,1,0});
    std::shared_ptr<Hittable> box1 = std::make_shared<Box>(Vec3{0, 0, 0}, Vec3{165, 330, 165}, white);
    box1 = std::make_shared<Hittable_Rotate_Y>(box1, 15);
    box1 = std::make_shared<Hittable_Translate>(box1, Vec3{265,0,295});
    std::shared_ptr<Hittable>  box2 = std::make_shared<Box>(Vec3{0,0,0}, Vec3{165,165,165}, white);
    box2 = std::make_shared<Hittable_Rotate_Y>(box2, -18);
    box2 = std::make_shared<Hittable_Translate>(box2,Vec3{130,0,65});
    objects.add(std::make_shared<ConstantMedium>(box1, boxVolumeShader, 0.1));
    objects.add(std::make_shared<ConstantMedium>(box2, boxVolumeShader, 0.1));
...
int main() {
    ...
    const auto max_samples = 500;


    auto world = cornell_box_volume_transformation();
    ...
    stbi_write_jpg("box.jpg",image_width, image_height,3, imageData.data(),100);
